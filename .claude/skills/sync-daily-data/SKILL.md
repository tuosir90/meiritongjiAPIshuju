...
- 列顺序：日期 | 火山引擎消费 | 云雾api消费 | 糖果姐姐api | APIMart | 馒小白 | 章鱼哥AI | 总生图数
...
- 章鱼哥AI 消费金额（数字，为空时按0处理；1:1 倍率，页面显示即真实消费）
- 总生图数（整数）
...
- totalCost必须等于六个API费用之和（火山引擎、云雾API、糖果姐姐API、APIMart、馒小白、章鱼哥AI）
- 记录按日期降序排列（最新的在最前面）
- 版本号必须递增一次，否则用户端不会更新
